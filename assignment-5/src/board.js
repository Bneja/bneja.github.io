...
function sound(card,src,img){
    
    if (card.img_id==img){
        document.getElementById("sfx").src=src
...
const boardC = {
    ...
    <div class="cardboard">
        ...
        <audio id="sfx" src="" style="display: none;"></audio>
    </div>
    `,
    data: function() {
    ...
    methods:{
        tryFlip:function(card){
            if (this.flippedCardCount===0){
                ...
                sound(card,"src/oke.mp3","N")
                sound(card,"src/doit.mp3","O")
